...
{
    public partial class FSfpCorrector : Form
    {
        ...
        public FSfpCorrector()
        {
            InitializeComponent();

            if (ucSfpCorrector.SetQsfpI2cReadCBApi(_I2cRead) < 0) {
                MessageBox.Show("ucQsfpCorrector.SetQsfpI2cReadCBApi() faile Error!!");
                return;
            }
            if (ucSfpCorrector.SetQsfpI2cWriteCBApi(_I2cWrite) < 0) {
